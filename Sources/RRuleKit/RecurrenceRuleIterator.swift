...
///
/// The `RecurrenceRuleIterator` protocol defines the requirements for creating iterators
...
public protocol RecurrenceRuleIterator: IteratorProtocol<Date> {
  ...
  var recurrenceEnd: RecurrenceRule.End { get }
  ...
  ///
  /// - Parameters:
  ///   - rrule: The `RecurrenceRule` defining the recurrence pattern.
  ///   - startDate: The date from which to start the iteration.
  ///   - calendar: The calendar to use for date calculations.
  ///
  /// - Note: An optional instance of a `RecurrenceRuleIterator`. Returns `nil` if the `RecurrenceRule` is incompatible
  ///   with the iterator (e.g., a daily iterator cannot handle a weekly frequency).
  init?(rrule: RecurrenceRule, from startDate: Date, in calendar: Calendar)
}
